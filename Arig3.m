function [E_1,a_m,l_opt,l_m_opt,P_opt,f_u_opt]= Arig3(W,Q,H,q_E,K,N,tt,B_k,B_m,np,P_0,h,h_m,u,u_m,y,L,C,P_max)
%P_max = 0.7;%UAV1最大功率
Y_c = 1e-24;%计算能耗的参数
...
for k = 1:1:K
    for n=1:1:N
        if ((u_m(k,n+1)*log(2))/(tt*q_E)-np/h_m(n+1)>P_max)
            P_opt(k,n+1) = P_max;
        elseif (u_m(k,n+1)*log(2))/(tt*q_E)-np/h_m(n+1)<0
            P_opt(k,n+1) = 0;
        else
            P_opt(k,n+1) = ((u_m(k,n+1)*log(2))/(tt*q_E)-np/h_m(n+1));
        end
...
for n = 1:1:N
    min=inf;
    for k=1:1:K
        z = (Q(1,n+1)-W(1,k))^2+(Q(2,n+1)-W(2,k))^2;
        if z<min
            min=z;%保存最小距离值
            a=k;%保存距无人机最近的用户编号
        end
